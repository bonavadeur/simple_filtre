    ...
    @if ($product && $status)
        <section id="cart_items">
            @foreach ($product as $key => $value)
                                    ...
                                    @foreach ($value as $key2 => $value2)
                                        <tr>
                                            <td class="cart_product" style="width:150px;">
                                                <img width="100px" src="{{ '../upload/product/' . $value2['image'] }}"
                                                    alt="">
                                            </td>
                ...
                <div style="color:rgb(192, 7, 7);font-weight:bold;font-size:20px;position:absolute;right:50px;">Tổng thanh
                    toán:
                    <?php
                    $final = 0;
                    foreach ($value as $key2 => $value2) {
                ...
                <?php
                if ($status[$key] == 2) {
                echo '<div style="color:green;font-weight:bold;font-size:20px;position:absolute;right:50px;">Đặt hàng thành
                    công</div>';
                }
                if ($status[$key] == 1) {
                echo '<div
                    style="color:rgb(192, 7, 7);font-weight:bold;font-size:25px;right:0px;position:absolute;right:50px;">
                    Đang chờ xử lí</div>' . '<br><br>' . '<button class="btn" style="position:absolute;right:50px;"><a
                        href="' . URL::to('confirm/' . $key) . '">Xác nhận đơn hàng</a></button>';
                }
                ?>
                <br><br>
